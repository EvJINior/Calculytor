...
function calculator_f(idElementaBody) {
    ...
    //let table_Numb = document.createElement('table');       
    table.setAttribute("border", "1");
    table.setAttribute("cellspacing", "2");      
    let button_equally = document.createElement('button');
    let img = document.createElement('img');
    table.appendChild(button_equally);
    button_equally.appendChild(img);
    img.setAttribute("src", "картинки для калькуляторо/Арифмет. знаки/equally.png");
    ...
    for (let i = 0; i < picturesNumbers.length; i++) { //Создает кнопки от 0 до 9.        
        let buttonNumbers = "button" + i;                          
        buttonNumbers = document.createElement('button');
        let img = document.createElement('img');                
        table.appendChild(buttonNumbers);
        //table_Numb.setAttribute("class", "numb_style");
   ...
   // idElementaBody.appendChild(table_Numb);
